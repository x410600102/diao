biz.helpercopy = {
	...
	reqRegionHtml(options) {
		...
		const op = $.extend({ prev: '',city: '',type:"", callback: null, tpl: '' }, options);
		let citysAllInfo = citys.citylist;
		let Info = [];
		if(op.type == "prev"){
			Info = [];
			for(let i = 0;i<citysAllInfo.length;i++){
				let obj = {
					code:citysAllInfo[i].p,
					name:citysAllInfo[i].p
				}
				Info.push(obj);
				console.log(citysAllInfo)
			}
		}else if(op.type == "city"){
			Info = [];
			for(let i = 0;i<citysAllInfo.length;i++){
				if(citysAllInfo[i].p == op.prev){
					for(let j = 0;j<citysAllInfo[i].c.length;j++){
						let obj = {
							code:citysAllInfo[i].c[j].n,
							name:citysAllInfo[i].c[j].n
						}
						Info.push(obj);
					}
				}
			}
		}else if(op.type == "dist"){
			Info = [];
			for(let i = 0;i<citysAllInfo.length;i++){
				if(citysAllInfo[i].p == op.prev){
					for(let j = 0;j<citysAllInfo[i].c.length;j++){
						if(citysAllInfo[i].c[j].n== op.city){
							for(let t = 0;t<citysAllInfo[i].c[j].a.length;t++){
								let obj = {
									code:citysAllInfo[i].c[j].a[t].s,
									name:citysAllInfo[i].c[j].a[t].s
								}
								Info.push(obj);
							}
						}
		...
		let html = template.render(op.tpl, { list: Info});
		op.callback(html);
		...
		$form.requestList = (loadMore) => {
			$.ajax({
				...
				success: (res) => {
					...
					if ($.isAjaxStatusOk(json)) {
						$form.listTotal(json.Title, json.Entity);
                        if ($form.total) {
						...
						$form.find("button[name=config]").on("click",function(event){
							...
							if($form.find("input:checked").length > 0){
								data = JSON.parse($($form.find("input:checked").parentsByTag("li").get(0)).attr("data-item"))
							}else{
								data ={
									"KHID":"",
									"QC":"无"
							...
							op.callback && op.callback.call($(op.target), data);
							$.filterPanel.close();
						})
					}
				},
...
//						let $items = $(_html);
						...
						$form.find(".back-button").on("click",function(event){
							$.technological.close();
						})
